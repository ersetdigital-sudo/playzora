...
export function GamePricingCard({ game, nominals }: GameCardProps) {
  ...
  const inputClass = "bg-raise border border-white/[0.06] rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#7c5cff]/40 transition";
      ...
      <div className="hairline rounded-2xl bg-panel overflow-hidden">
              ...
              {editing === item.id ? (
                ...
                <>
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="shrink-0 w-9 h-9 flex items-center justify-center rounded-lg bg-white/[0.04] text-white/30 hover:text-[#7c5cff] hover:bg-[#7c5cff]/10 transition disabled:opacity-50"
                    title="Edit"
                  >
        ...
        <form onSubmit={handleAdd} className="flex gap-2 p-4 border-t border-white/[0.04]">
          ...
          <button
            type="submit"
            disabled={loading}
            className="shrink-0 px-4 py-2.5 text-sm font-semibold rounded-lg transition disabled:opacity-50 btn-violet"
          >
            {loading ? "…" : "Tambah"}
          </button>
        </form>
