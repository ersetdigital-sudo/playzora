...
function buildGame(slug: string) {
  console.log("[top-up] buildGame called with slug:", slug);
  const g = GAMES.find((x) => x.slug === slug);
  if (!g) {
    console.log("[top-up] game not found for slug:", slug);
    return null;
  }
  console.log("[top-up] game found:", g.name);
  return {
    id: `fb-${slug}`,
    slug: g.slug,
    name: g.name,
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  console.log("[top-up] generateMetadata slug:", slug);
...
export default async function TopUpPage({ params }: PageProps) {
  try {
    const { slug } = await params;
    console.log("[top-up] rendering page for slug:", slug);
    const game = buildGame(slug);
    if (!game) {
      console.log("[top-up] game not found, calling notFound()");
      notFound();
    }

    console.log("[top-up] building crumbs");
    const crumbs = [
      { label: "Home", href: "/" },
    ...
    console.log("[top-up] rendering JSX");
    return (
      <>
        <Header />
    ...
    console.error("[top-up] FATAL ERROR:", err);
    throw err;
  }
}
